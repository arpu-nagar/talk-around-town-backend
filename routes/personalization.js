...
import {
    ...
    CATEGORY_RESPONSES,
} from '../utils/parentingGuardrails.js'; // Add this import
// import {
//   parseStrictParentingRequest,
//   ALLOWED_CATEGORIES,
...
router.post('/enhanced-tips', authenticateJWT, async (req, res) => {
    try {
        ...
        // 0) Strict format & category enforcement
        // const parsed = parseStrictParentingRequest(prompt);
        // if (!parsed.ok) {
        //     return res.status(400).json({
        //         error: parsed.error,
        ...
        // // 1) Parenting guardrails (safety/scope)
        // const validation = validateParentingQuery(parsed.strictPrompt);
        ...
        // if (!validation.isValid) {
        //     console.log(
        //         `🚫 Rejected ${validation.type} query from user ${userId}: "${prompt}"`,
        //     );
        ...
        console.log(
            `✅ Validated parenting query from user ${userId}: "${prompt}" (mode: ${generateMode})`,
        );
        ...
        if (generateMode === 'generate') {
            ...
            console.log(
                `🔍 Trying database search first for parenting query: "${prompt}"`,
            );
            ...
            console.log(
                `🤖 No database results, generating AI parenting tips for "${prompt}"`,
            );
            result =
                await personalizationService.generatePersonalizedTipsForQuery(
                    userId,
...
router.post('/generate-tips', authenticateJWT, async (req, res) => {
    try {
        ...
        console.log(
            `🤖 AI generation request from user ${userId} for validated parenting query: "${prompt}"`,
        );
