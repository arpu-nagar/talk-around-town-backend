...
import { validateParentingQuery } from '../utils/parentingGuardrails.js'; // Add this import
...
router.post('/enhanced-tips', authenticateJWT, async (req, res) => {
  try {
    ...
    // GUARDRAILS: Validate that this is a parenting-related query
    const validation = validateParentingQuery(prompt);
    if (!validation.isValid) {
      console.log(`🚫 Rejected ${validation.type} query from user ${userId}: "${prompt}"`);
      ...
      return res.status(400).json({
        ...
        suggestions: [
          "Try asking about bedtime routines",
          "Ask about activities for your child's age",
    ...
    console.log(`✅ Validated parenting query from user ${userId}: "${prompt}" (mode: ${generateMode})`);
    ...
    } // Hybrid mode (default): Try database first, then AI generate if needed
   else {
    console.log(`🔍 Trying database search first for parenting query: "${prompt}"`);
    // ✅ DB-first (returns numeric tip ids)
    result = await personalizationService.getContextualPersonalizedTips(
      userId,
      prompt,
      3,
    ...
    // 🤖 fallback to AI if DB had nothing
    console.log(`🤖 No database results, generating AI parenting tips for "${prompt}"`);
    result = await personalizationService.generatePersonalizedTipsForQuery(userId, prompt, 3, contentPreferences);
    ...
    // Ultimate fallback
    console.log(`❌ All methods failed for parenting query: "${prompt}"`);
    res.status(200).json({
      tips: [],
...
router.post('/generate-tips', authenticateJWT, async (req, res) => {
  try {
    ...
    console.log(`🤖 AI generation request from user ${userId} for validated parenting query: "${prompt}"`);
      ...
      generationStats: {
        avgPersonalMatch:
          result.tips.length > 0
            ? result.tips.reduce((sum, tip) => sum + tip.personal_match, 0) /
              result.tips.length
            : 0,
      },
    });
...
router.post('/survey', authenticateJWT, async (req, res) => {
  try {
    const userId = req.user.id;
    const { surveyData, completedAt } = req.body;
...
router.post('/enhanced-tips-survey', authenticateJWT, async (req, res) => {
  try {
    ...
    if (surveyData.length > 0) {
      ...
      const userPrefs = safeJSONParse(survey.content_preferences);      // FIXED
      const userChallenges = safeJSONParse(survey.challenge_areas);     // FIXED
      const userGoals = safeJSONParse(survey.parenting_goals);          // FIXED
...
function buildSurveyContext(survey) {
  const contentPrefs = safeJSONParse(survey.content_preferences);   // FIXED
  const challenges = safeJSONParse(survey.challenge_areas);         // FIXED
  const goals = safeJSONParse(survey.parenting_goals);              // FIXED
...
async function updateCombinedPreferenceProfile(userId) {
  ...
  try {
    ...
    if (interactionEmbedding) {
      ...
      combinedEmbedding = surveyAverage.map((sv, i) => 
        sv * surveyWeight + interactionEmbedding[i] * (1 - surveyWeight)
      );
    } else {
