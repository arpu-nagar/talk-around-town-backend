...
class PersonalizationService {
  ...
  async getContextualPersonalizedTips(userId, query, limit = 10, contentPreferences = []) {
    try {
      console.log(
      ...
      if (userProfile.length > 0 && userProfile[0].preference_embedding) {
        if (typeof userProfile[0].preference_embedding === 'string') {
          userPreference = JSON.parse(userProfile[0].preference_embedding);
      ...
      for (const tipEmbedding of tipEmbeddings) {
        try {
          ...
          const lambda = 0.25;
          let combinedScore = querySimilarity * 0.6 + personalizedScore * 0.4;
          if (dislikeCentroid) {
      ...
      const relevantTips = recommendations.filter((t) => t.query_relevance > 0.3);
      const finalTips = relevantTips.slice(0, limit);

      console.log(`✅ Found ${finalTips.length} contextual personalized tips for "${query}"`);
      console.log(`   Top tip relevance: ${finalTips[0]?.query_relevance ?? 'N/A'}`);
  ...
  async generatePersonalizedTipsForQuery(userId, query, limit = 5, contentPreferences = []) {
    try {
      console.log(
      ...
      console.log(`✅ Generated ${finalTips.length} personalized tips for "${query}"`);
      if (finalTips.length > 0) {
        console.log(`   Best match score: ${finalTips[0].personal_match}`);
        console.log(
...
async generateTipsWithAI(query, preferenceContext = '', count = 10, contentPreferences = []) {
    const maxRetries = 3;
    let lastError = null;
    
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🤖 AI generation attempt ${attempt}/${maxRetries} for query: "${query}"`);
        ...
        if (Array.isArray(contentPreferences) && contentPreferences.length) {
          const prefsText = contentPreferences.join(' and ');
          prompt += ` Focus primarily on ${prefsText} content.`;
        }
        if (preferenceContext) {
          prompt += ` ${preferenceContext} Please tailor the tips to match these preferences.`;
        }
  ...
  Focus specifically on "${query}" and make each tip unique and useful.`;
        ...
        console.log('🤖 Raw OpenAI response preview:', raw.substring(0, 200) + '...');
        ...
        try {
          ...
          console.log('⚠️ Direct JSON parse failed, trying fallbacks...');
          ...
          try {
            ...
            console.log('⚠️ Regex extraction failed, trying repair...');
            ...
            try {
              ...
              if (firstBracket > 0) {
                repairedJson = repairedJson.substring(firstBracket);
              }
              
              // Remove any text after the last ]
              const lastBracket = repairedJson.lastIndexOf(']');
              if (lastBracket > 0) {
                repairedJson = repairedJson.substring(0, lastBracket + 1);
              }
              ...
              console.log('🛡️ Returning fallback tips due to JSON parse failure');
        ...
        console.log(`✅ Successfully generated ${formattedTips.length} AI tips for "${query}"`);
  ...
  async getPersonalizedTips(userId, limit = 10) {
    try {
      ...
      if (tipEmbeddings.length === 0) {
        console.log(`No available tips for user ${userId}, returning empty array`);
        return [];
      }
